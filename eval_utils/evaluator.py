...
from eval_utils.metrics import create_category_mask, calc_metrics
from eval_utils.visualization import show_plots, show_overview, get_result_string, show_epe_categories
from eval_utils.io import readPFM
from eval_utils.io import load_LIU_data, load_MiddV3_data, write_eval, scale_disp
...
class EvaluationItem():
	...
	def show(self, epethresh=20, saveDir=False):
		"""
		Shows a figure summarizing the data, output and resulting metrics for this item.
		Also, saves the figure if asked to
		"""
		comb, cmap = create_category_mask(self.wta, self.gt)
		show_overview(self.wta, self.gt, self.output, self.epe_out, self.rgb, comb, cmap, epethresh, self.details, saveDir=saveDir, show_now=True)
...
class Evaluator():
	...
	def create_fig_path(self, item):
		...
		if self.set == 'liu':
			...
			return 'eval_results/LIU/{}/{}_{}.png'.format(pair, self.mode.upper(), scene)
		elif self.set == 'middv3':
			return 'eval_results/MiddV3/{}_{}.png'.format(self.mode.upper(), item)
		else:
			raise Exception
