...
def load_LIU_data(root, pair, mode, item):
    ...
    if mode == 'saab':
        ...
        output = imageio.imread(root + pair + '/left_inpaint_disparity/' + item + '.png')
...
def load_MiddV3_data(root, folder, mode, *args):
    wta, scale =        readPFM(root + folder + '/disp_WTA.pfm')
    if mode == 'saab':
        output, scale = readPFM(root + folder + '/disp_WTA.pfm')
    elif mode == 'sdr':
...
def scale_disp(d):
    return -d.astype('float32') / 64 + 350
